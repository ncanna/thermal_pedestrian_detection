...
model = get_model_instance_segmentation(3) #GET THIS FOR NUMBER OF CLASSES
model.to(device)
params = [p for p in model.parameters() if p.requires_grad]
optimizer = torch.optim.adam(params, lr = 0.005, momentum = 0.9, weight_decay = 0.0005) #Adjust these
...
EPS = 1e-6
def iou(prd: torch.Tensor, annot: torch.Tensor):
    intersection = torch.logical_and(annot, prd)
    union = torch.logical_or(annot, prd)
    iou = torch.sum(intersection)/torch.sum(union)

    thresh = torch.clamp(20* (iou - .5), 0, 10).ceil() / 10
...
for i in range(len(imgs)):
    iou = iou(preds[i], annotations[i])
    ious.append(iou)
mean = torch.mean(torch.stack(ious))
print(mean)
#print(preds)
